const API_KEY = '058cde373a2455868cb42eb28d395b9d';
...
export default {
	getHomeList: async () => {
		return [
			{
				slug: "originals",
				title: "Top séries para você assistir",
				items:
					await basicFetch
				(
					`/discover/tv?with_network=213&sort_by=vote_average.desc&vote_count.gte=100&language=en-US&page=1&api_key=${API_KEY}`
				),
			},
			{
				slug: "treding",
			...
			},
			{
				...
				items: await basicFetch(`/discover/tv?with_genres=16&language=en&api_key=${API_KEY}`)
			},
			{
				slug: 'action',
				title: 'Ação',
